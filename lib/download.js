...
module.exports.download = async (opts) => {
	...
	await Promise.all(
		prebuildFolders.map(async (prebuildFolder) => {
			const files = await fsReadDir(
				path.join(prebuildRoot, prebuildFolder),
			);
			filesToCopy.push(
				...files.map((file) => path.join(prebuildFolder, file)),
			);
		}),
	);
	...
	await Promise.all(
		filesToCopy.map((file) => {
			const newFileName = file.includes("zeromq.")
				? file.replaceAll("zeromq.", "node.napi.")
				: file;
			console.info(
